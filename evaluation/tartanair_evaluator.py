...
class TartanAirEvaluator:
    ...
    def evaluate_one_trajectory(self, gt_traj, est_traj, scale=False, title='', save_path = None):
        """
        ...
        ate_score, gt_ate_aligned, est_ate_aligned, std = self.ate_eval.evaluate(gt_traj, est_traj, scale)

        plot_traj(np.matrix(gt_ate_aligned), np.matrix(est_ate_aligned), vis=False, savefigname=f"{save_path}/%s.png"%title, title=title)
        ...
        return {'ate_score': ate_score, 'rpe_score': rpe_score, 'kitti_score': kitti_score, "ate_std": std}
